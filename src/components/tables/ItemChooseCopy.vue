<template>
    <Dialog header="Подбор номенклатуры" class="border" v-model:visible="displayDialog" :modal="true" :closable="false"
      :contentStyle="{height: '100%'}" :style="{width: '90%', height: '900px'}">
       <!-- @keyup.enter="addItemsToDoc">  -->
        <template #header>

        </template>
        <div class="container" @keyup="onKeyUp($event)">
            <div class="col-12 md:col-3">
                <i>{{ itemNameFilter }}</i><Button icon="pi pi-times" class="p-button-rounded p-button-text p-button-plain p-button-sm" @click="clearFilter"/>
            </div>
